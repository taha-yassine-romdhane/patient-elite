...
export default function PaymentSummary({ payments, totalAmount, onRemovePayment }: PaymentSummaryProps) {
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {payments.map((payment, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {payment.method === "CASH" && payment.cashTotal && payment.cashAcompte && (
                    ...
                    {payment.method === "CHEQUE" && payment.chequeNumber && (
                      <div className="text-xs">
                        <div>N° chèque: {payment.chequeNumber}</div>
                        {payment.chequeDate && (
                          <div className="text-blue-600 font-medium">
                            📅 Date: {new Date(payment.chequeDate).toLocaleDateString('fr-FR')}
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    {payment.method === "TRAITE" && payment.traiteDueDate && (
                      <div className="text-xs">
                        <div className="text-blue-600 font-medium">
                    ...
                    {!["CASH", "CHEQUE", "CNAM", "TRAITE"].includes(payment.method) && !payment.notes && "-"}
                  </td>
              ...
              <tr className="bg-gradient-to-r from-purple-50 to-purple-100">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-lg font-bold text-purple-800">
